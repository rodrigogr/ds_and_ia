...
def predict(body: request_body):
    ...
    predict_id = pd.DataFrame(body.model_dump(), index=[1])
    ...
    # Deixar a K melhores Features - Consultado no notebook quais melhores features selecionadas pelo modelo (kbest)
    ...
    return jsonify({'obesidade': y_pred.to_list()})
